...
//           borderRadius:12, padding:"14px 18px", color:"#a5f3fc",
//           fontSize:13, fontFamily:"monospace", whiteSpace:"pre-wrap", lineHeight:1.7, marginBottom:14,
//         }}>{preset.code}</pre>
//         <div style={{
...
//         }}>
//           {preset.description || "Strategy สำเร็จรูป — ใช้ได้ทันที"}
//         </div>
...
//           cursor:"pointer", fontFamily:"'Cinzel', serif", fontSize:15, fontWeight:700,
//         }}>SELECT ✓</button>
//       </div>
//     </div>
//   );
...
//     setLoading(true);
//     const result = await checkStrategy(code);
...
//           placeholder="your strategy..." rows={8}
//           style={{
...
//     setLoading(true);
//     const result = await checkStrategy(customCode);
//     setCheckResult(result);
//     setLoading(false);
...
//             <button key={ps.id} onClick={() => onViewPreset(ps)} style={{
...
//             </button>
//           ))}
//           <button onClick={() => onViewCustom()} style={{
//             padding:"7px 10px", borderRadius:9, textAlign:"left",
//             border:"1px solid rgba(196,181,253,0.3)",
//             background:"rgba(196,181,253,0.07)", color:"#c4b5fd",
//             cursor:"pointer", fontFamily:"'Cinzel', serif", fontSize:11,
//             display:"flex", justifyContent:"space-between",
//           }}>
...
//       const n=[...prev]; n[activeSlot]={ ...n[activeSlot], [minionId]: Number(val) }; return n;
//     });
//   };
...
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ mode: gameState.mode?.toUpperCase() || "DUEL" }),
//       });

//       // 2. Spawn minion P1 ทุกตัว
//       const spawnPositions = [
//         { row:1,col:1 },{ row:1,col:2 },{ row:2,col:1 },{ row:2,col:2 },{ row:1,col:3 }
//       ];
//       for (let i = 0; i < slots.length; i++) {
//         const minionId = slots[i];
//         if (!minionId) continue;
//         const defense  = defenseValues[i]?.[minionId] ?? 10;
//         const strategy = strategyValues[i] || "move up";
//         const pos      = spawnPositions[i] || { row:1,col:1 };
//         await fetch(`${API}/spawn`, {
//           method: "POST",
//           headers: { "Content-Type": "application/json" },
//           body: JSON.stringify({
//             playerId:"p1", kindName:minionId,
//             row:pos.row, col:pos.col, defense, strategy,
//           }),
//         });
//       }

//       // 3. บันทึก state แล้วไปหน้า game
//       updatePlayer(0, {
//         selectedMinions: slots,
//         minionDefense:   defenseValues[activeSlot],
//         strategy:        strategyValues[activeSlot],
//       });
//       onNext("game");
//     } catch (e) {
...
//                 cursor: usedOther ? "not-allowed" : "pointer", opacity: usedOther ? 0.3 : 1,
...
//                     background:"rgba(196,181,253,0.1)", color:"#fff",
//                     fontSize:20, fontWeight:700, outline:"none", fontFamily:"monospace", textAlign:"center",
//                   }}
//                 />
//               </div>
...
//             fontSize:"clamp(18px,1.2vw,13px)", fontWeight: curMinionId===m.id ? 700 : 400,
//           }}>{m.name}</div>
//         ))}
//       </div>
...
//         borderRadius:40, color: allReady ? "#fff" : "rgba(255,255,255,0.3)",
//         cursor: allReady ? "pointer" : "not-allowed",
//       }}>
//         {starting ? "กำลังสร้างเกม..." : allReady ? "GAME START →" : `ยังไม่ครบ (${filledSlots}/${minionCount} ตัว · ${filledStrategy}/${minionCount} strategy)`}
//       </button>
//     </div>
//   );
// }
...
export default function CollectionScreen({ onNext, onBack }) {
  const { gameState, updatePlayer, setGameState } = useGame();
  const minionCount = gameState.minionCount || 1;
  ...
  // ── GAME START → สร้างเกมอย่างเดียว ไม่ spawn ──────────────
  // การ spawn จะทำใน GameBoardScreen (setup phase) แทน
  const handleStart = async () => {
    setStarting(true);
    try {
      // สร้างเกมที่ backend
      ...
      // เก็บ minion config ไว้ใน context ให้ GameBoard ใช้ตอน spawn
      // slots[i] = minionId, defenseValues[i][minionId] = defense, strategyValues[i] = strategy
      const minionConfigs = slots.map((minionId, i) => ({
        minionId,
        defense: defenseValues[i]?.[minionId] ?? 10,
        strategy: strategyValues[i] || "move up",
      }));

      updatePlayer(0, {
        selectedMinions: slots,
        minionDefense:   defenseValues[activeSlot],
        strategy:        strategyValues[0] || "move up",
        minionConfigs,   // ← GameBoard จะเอาไปใช้ตอน setup spawn
      });
