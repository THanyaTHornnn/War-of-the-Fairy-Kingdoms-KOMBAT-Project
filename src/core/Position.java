...
/**
 * Hex grid 8x8 สำหรับ KOMBAT
 * ตรวจสอบจากสเปค: (1,1) downright → (2,2) ✓
 * → pointy-top, odd-row offset
 *
 * Neighbor table:
 *   odd row:  UP=(r-1,c) DOWN=(r+1,c) UPRIGHT=(r-1,c+1) DOWNRIGHT=(r+1,c+1) UPLEFT=(r-1,c)   DOWNLEFT=(r+1,c)
 *   even row: UP=(r-1,c) DOWN=(r+1,c) UPRIGHT=(r-1,c)   DOWNRIGHT=(r+1,c)   UPLEFT=(r-1,c-1) DOWNLEFT=(r+1,c-1)
 */
public class Position {
    private final int row;
    private final int col;
    ...
    // odd-row offset → cube → hex distance
    public int distanceTo(Position other) {
        int x1 = this.col - (this.row - (this.row & 1)) / 2;
        int z1 = this.row;
        int y1 = -x1 - z1;
